...
def sample_data(df, seed: int = 42):
    # 正/负样本划分
    p_df = df[df.click.isin([1])]
    n_df = df[df.click.isin([0])]
    del df

    # 负样本采样：若数量不足，按可用最大值采样
    need_n = len(p_df) * 2
    take_n = min(len(n_df), need_n)
    n_df = n_df.sample(n=take_n, random_state=seed)

    # 这里替换掉 p_df.append(n_df)
    df = pd.concat([p_df, n_df], axis=0, ignore_index=True)
    del p_df, n_df

    # 打乱（加 random_state），并重置索引
...
class cold_reset_df(object):
    ...
    def fit_transform(self, df1, df2):
        print("=" * 10, "Resetting user ids and item ids in DataFrame", "=" * 10)
        # 用 pd.concat 代替 append（pandas 2.x）
        df = pd.concat([df1['user_id'], df2['user_id']], axis=0, ignore_index=True)
